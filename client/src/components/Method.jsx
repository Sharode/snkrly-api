...
const Method = ({ attrs, paths, objAttr }) => {
	const attributes = attrs.map((attr) => (
		<div key={attr.id} className="my-4">
			{attr.attr !== 'id' && <div className="mb-4 border-t w-full" />}
			<h4 className="font-bold">
				{' '}
				{attr.attr} <span className="text-xs font-medium text-red-600">{attr._obj}</span>
			</h4>
			<p>{attr.text}</p>
		</div>
	));
	...
	const path = paths.map((path) => (
		<tr className="text-sm " key={path.id}>
			<th className={`px-4 pb-2 align-top text-right text-${colors[path.id]}-600`}>{path.method}</th>
	...
	return (
		<Fragment>
			<section className="flex flex-col md:flex-row">
				<div className="md:w-1/2">
					...
					<p className="text-base">
						This is an object representing the snkrs curated collection. You add and update items in the
						collection if there is something is misrepresented or not avaliable in the collection but should
						be
					</p>
				</div>
			...
			<div className="mt-10 border-t w-full md:w-1/2" />
